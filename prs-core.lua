...
local function get_dropped_and_added_affects(old_affects, new_affects)
    ...
    if new_affects then table.sort(new_affects, function(left, right)
      return left.name < right.name 
    end) end
    if old_affects then table.sort(old_affects, function(left, right)
      return left.name < right.name 
    end) end
...
function core.on_gmcp_char(event, args)
    ...
    if diff_table.affects then
        if not gmcp.Char.affects then gmcp.Char.affects = {} end
