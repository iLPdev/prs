...
function updateBattleEntities(participants, container, labels, bars, side)
  for i, participant in ipairs(participants) do
    ...
    labels[i].mainText:cecho(PRSutil.getCechoColor(participant.tag .. " " .. participant.name .. "\n{{KL{{W" .. participant.level .. "{{w " .. affects))
    ...
    labels[i].targetText:cecho(PRSutil.getCechoColor(targetText))
...
function setGaugesForParticipant(participant, containerLabel, bars, i, side)
  ...
  bars[i].hpBar = createBattleGauge(participant.hp, containerLabel.main, i, bars[i].hpBar, side, "HP", "#63e2b7", startY)
  bars[i].enBar = createBattleGauge(participant.en, containerLabel.main, i, bars[i].enBar, side, "EN", "#70c0e8", startY + (PRSbattle.barHeight + PRSbattle.barGap))
  bars[i].stBar = createBattleGauge(participant.st, containerLabel.main, i, bars[i].stBar, side, "ST", "#f2c97d", startY + (PRSbattle.barHeight + PRSbattle.barGap) * 2)
end
...
function createBattleGauge(currentAmount, containerLabel, i, current, side, name, progressColor, y)
  ...
  -- set styling
  ret.progressBar.front:setStyleSheet(
    string.format(
      [[
    background-color: %s;
...
PRSbattle.repositioningHandlerId = registerAnonymousEventHandler("AdjustableContainerRepositionFinish", repositioningContainer)
